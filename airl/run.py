...
def _apply_overrides(a, res_cfg, algo_cfg):
    if a.data_path is not None: res_cfg["data_path"] = a.data_path
    if a.date_column is not None: res_cfg["columns"]["date"] = a.date_column
    if a.state_variables is not None: res_cfg["columns"]["state"] = a.state_variables
    if a.use_month_encoding is not None: res_cfg["columns"]["use_month_encoding"] = a.use_month_encoding
    for k, key in (("split_train", "train"), ("split_val", "val"), ("split_test", "test")):
        if getattr(a, k) is not None: res_cfg.setdefault("split", {})[key] = getattr(a, k)
    for k in ("storage_variable", "inflow_variable"):
        v = getattr(a, k)
        if v is not None: res_cfg.setdefault("columns", {})[k.replace("_variable", "")] = v
    ...
    if mb: res_cfg.setdefault("reservoir", {}).setdefault("mass_balance", {}).update(mb)

    dev = a.device
    if dev is not None:
        algo_cfg["bc_tuning"]["runtime"]["device"] = dev
        algo_cfg["airl_tuning"]["runtime"]["device"] = dev
    if a.bc_n_trials is not None: algo_cfg["bc_tuning"]["optuna"]["n_trials"] = a.bc_n_trials
    if a.bc_n_jobs is not None: algo_cfg["bc_tuning"]["runtime"]["num_workers"] = a.bc_n_jobs
    if a.airl_n_trials is not None: algo_cfg["airl_tuning"]["optuna"]["n_trials"] = a.airl_n_trials
    if a.airl_n_jobs is not None: algo_cfg["airl_tuning"]["runtime"]["num_workers"] = a.airl_n_jobs
...
def main():
    ...
    _apply_overrides(a, res_cfg, algo_cfg)
